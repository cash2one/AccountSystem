...
import com.snda.grand.mobile.as.account.impl.MemcachedAccessorService;
import com.snda.grand.mobile.as.exception.InvalidAccessorException;
import com.snda.grand.mobile.as.exception.UnauthorizedInternalAccessException;
...
public class AccessorAuthenticateFilter implements ContainerRequestFilter {
	...
	public AccessorAuthenticateFilter(MemcachedAccessorService accessorService) {
		...
		LOGGER.info("AccessorAuthenticateFilter initialized.");
	}
	...
	public ContainerRequest filter(ContainerRequest request) {
		...
		if (request.getRequestUri().toString().startsWith(request.getBaseUri() + IGNORE_OAUTH_REQUEST_PATH)) {
			LOGGER.info("Ignore checking accessor auth for path : {}.", request.getRequestUri());
			return request;
		}
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Accessor's access key is {}.", accessKey);
		}
		return request;
	}
